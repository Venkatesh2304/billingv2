...
class ScanPendingBills(View):
    ...
    def get(self, request):
        ...
        if bill_no : 
            ...
            bills_info = [(obj.bill, "a", obj.sheet_id , (obj.bill_status is None) or (obj.bill_status == "scanned") ,  obj.status() ) for obj in queryset]
...
def sync_impact(request):
    ...
    loaded_qs = models.Bill.objects.filter(loading_time__date = datetime.date.today())
    from_date = loaded_qs.aggregate(min_date = models.Min('bill__date'))['min_date']
    ...
    for vehicle , bills in vehicle_bills_final.items() :
        loaded_bills = loaded_qs.filter(vehicle = vehicle).values_list("bill_id",flat=True)
        bills.extend(loaded_bills)
        if  vehicle.name_on_impact is None : 
                raise Exception("Vehicle name on impact is not set") 
        Billing().sync_impact(from_date,to_date,list(set(bills)),vehicle.name_on_impact)

    vehicle_bills_final = { vehicle.name : len(set(bills)) for vehicle,bills in vehicle_bills_final.items() }
    ...
    for vehicle in models.Vehicle.objects.all(): 
        ...
        qs_yesterday_bills = qs.filter(bill__date = yesterday)
        beats = qs_yesterday_bills.values_list("bill__beat",flat=True).distinct()
        for beat in beats : 
            beat_all_bills_count = yesterday_bills.filter(bill__beat = beat).count()
            beat_loaded_bills_count = qs.filter(bill__beat = beat).count()
