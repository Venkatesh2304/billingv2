        ...
        for day,rows in rows.groupby("day") : 
            ...
            while True :
                try :
                    ...
                    print(f"retrying {pdf_name}")
    os.chdir("..")

def batch_sales_return_pdf() : 
    b = Billing() 
    from_no = 118 
    to_no = 2200
    for no in range(from_no,to_no+1) : 
        salesreturn_no = f"SRT{str(no).zfill(5)}"
        for retry in range(3) : 
            try : 
                res = b.post("/rsunify/app/salesreturn/salesreturnPrint",data={"srhRefr":salesreturn_no,
                                                                        "reportType": "pdf","printerName": "TVS MSP 250 Star"}).json()
                durl = res["reportPath"]
                with open(f"DEVAKI_SALES_RETURN/{salesreturn_no}.json","w+") as f : 
                    f.write(json.dumps(res))
                b.download_file(durl,f"DEVAKI_SALES_RETURN/{salesreturn_no}.pdf")
                print(salesreturn_no)
                break 
            except Exception as e  :
                print(salesreturn_no,"failed for",retry)
                print(e)
                time.sleep(10)
                b = Billing()
